...
def network_graph(yearRange, AccountToSearch):
    ...
    for index in range(0,len(edge1)):
        edge1['Datetime'][index] = datetime.strptime(edge1['yearofentry'][index], '%d/%m/%Y')
    ...
    G = nx.from_pandas_edgelist(edge1, 'Source', 'Target', ['Source', 'Target', 'yearofentry', 'yearofexit'], create_using=nx.MultiDiGraph())
    nx.set_node_attributes(G, node1.set_index('Letter')['Name'].to_dict(), 'Name')
    nx.set_node_attributes(G, node1.set_index('Letter')['Area'].to_dict(), 'Area')
...
#        figure = {
#            "data": traceRecode,
#            "layout": go.Layout(title='Electricity System Cybersecurity', showlegend=False,
...
#        return figure
    ...
    for node in G.nodes():
        ...
        #hovertext = "name: " + str(G.nodes[node]['Name']) + "<br>" + "area: " + str(
        #    G.nodes[node]['Area'])
        text = node1['Letter'][index]
    ...
    figure = {
        "data": traceRecode,
        "layout": go.Layout(title='Connections between elements', showlegend=False, hovermode='closest',
                            margin={'b': 40, 'l': 40, 'r': 40, 't': 40},
                            xaxis={'showgrid': False, 'zeroline': False, 'showticklabels': False},
                            yaxis={'showgrid': False, 'zeroline': False, 'showticklabels': False},
                            height=600,
...
app.layout = html.Div([
    #########################Title
    html.Div([html.H1("Electricity System Cybersecurity")],
             className="row",
        ...
        children=[
            ...
            html.Div(
                ...
                children=[dcc.Graph(id="my-graph",
                                    figure=network_graph(YEAR, LETTER))],
            ),
